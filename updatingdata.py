...
def credential_gatherer():
    print("\n" + info + "Gathering credentials from file...")
...
def manual_update(tables_infos: dict, git: Git, file_names: list):
    ...
    if source.ok:
        ...
        if flag is True:
            print(info + "Update to 'toUpdate.yml' not needed (file names didn't changed!)")
...
def dispatcher(tables_infos: dict, to_update: dict, git: Git):
    ...
    if new_files != to_update:
        ...
        sleep(0.5)
        print("\n" + info + "Updating 'toUpdate.yml' not needed (file names didn't change!)")
...
def update_file_to_github(git: Git, old_file_name: str, new_file_name: str, key: str, file_content: str):
    ...
    if not old_file_name.endswith('.csv'):
        ...
        try:
            ...
            if e.status != 404:
                print("\t" + info +
                      " In the meantime, I was searching for a CSV file with the same name to ask you to remove it.\n"
